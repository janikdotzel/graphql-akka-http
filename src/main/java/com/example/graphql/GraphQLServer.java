...
public class GraphQLServer extends AllDirectives {
    ...
    public GraphQLServer(ActorSystem<?> system, ActorRef<UserRegistry.Command> userRegistryActor) {
        ...
        AsyncDataFetcher<CompletableFuture<UserRegistry.GetUserResponse>> userFetcher = async(environment -> {
            String name = environment.getArgument("name");
            System.out.println("Querying user " + name);
            CompletionStage<UserRegistry.GetUserResponse> response =
                    AskPattern.ask(userRegistryActor, ref -> new UserRegistry.GetUser(name, ref), askTimeout, scheduler);
            return response.toCompletableFuture();
        });
        AsyncDataFetcher<CompletableFuture<UserRegistry.Users>> allUsersFetcher = async(environment -> {
            System.out.println("Querying all users");
            CompletionStage<UserRegistry.Users> response =
                    AskPattern.ask(userRegistryActor, UserRegistry.GetUsers::new, askTimeout, scheduler);
            return response.toCompletableFuture();
        });
        AsyncDataFetcher<CompletableFuture<UserRegistry.ActionPerformed>> createUserMutation = async(env -> {
            String name = env.getArgument("name");
            int age = env.getArgument("age");
            String countryOfResidence = env.getArgument("countryOfResidence");
            System.out.println("Creating user " + name);
            CompletionStage<UserRegistry.ActionPerformed> response =
                    AskPattern.ask(userRegistryActor, ref -> new UserRegistry.CreateUser(new UserRegistry.User(name, age, countryOfResidence), ref), askTimeout, scheduler);
            return response.toCompletableFuture();
        });
        AsyncDataFetcher<CompletableFuture<UserRegistry.ActionPerformed>> deleteUserMutation = async(env -> {
            String name = env.getArgument("name");
            System.out.println("Deleting user " + name);
            CompletionStage<UserRegistry.ActionPerformed> response =
                    AskPattern.ask(userRegistryActor, ref -> new UserRegistry.DeleteUser(name, ref), askTimeout, scheduler);
            return response.toCompletableFuture();
        });
        ...
        RuntimeWiring runtimeWiring = newRuntimeWiring()
            ...
            .type("Mutation", builder -> builder
                .dataFetcher("createUser", createUserMutation)
